...
public:
    vector<int> findMaximums(vector<int>& v) {
        ...
        auto smaller_from_right = [&]()->void{
            for(int i = n-1;i>=0;i--){
                while(!st.empty() and st.back().first >= v[i]) st.pop_back();
                if(st.empty()) right_smaller[i] = n;
                else right_smaller[i] = st.back().second;
                st.push_back({v[i],i});
            }
            st.clear();
        };
        
        auto smaller_from_left = [&]()->void{
            for(int i = 0;i<n;i++){
                while(!st.empty() and st.back().first >= v[i]) st.pop_back();
        ...
        for(int i = 0; i < n; i++){
            ...
            assert(length >= 1 && length <= n);
            res[length - 1] = max(res[length - 1], v[i]);
        }
